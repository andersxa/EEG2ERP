...
dataset = ERPDataset(path='dataM/', split='train', processing='simple', num_samples=288, no_leakage=True, restricted=True).to('cuda')
...
if dataset.restricted:
    save_path = f'dataM/{dataset.processing}_num_trials_weights.pt'
else:
    save_path = f'dataM/{dataset.processing}_unrestricted_num_trials_weights.pt'
